...
public class RenderUtils {
    ...
    public static void renderRoundedQuad(MatrixStack matrices, double fromX, double fromY, double toX, double toY, double rad, double samples, Color c, boolean topLeft, boolean topRight, boolean bottomLeft, boolean bottomRight) {
        int color = c.getRGB();
        Matrix4f matrix = matrices.peek().getPositionMatrix();
        float f = (float) (color >> 24 & 255) / 255.0F;
        float g = (float) (color >> 16 & 255) / 255.0F;
        ...
        renderRoundedQuadInternal(matrix, g, h, k, f, fromX, fromY, toX, toY, rad, samples, topLeft, topRight, bottomLeft, bottomRight);

        RenderSystem.disableBlend();
        RenderSystem.setShaderColor(1f, 1f, 1f, 1f);
    }

    private static void renderRoundedQuadInternal(Matrix4f matrix, float cr, float cg, float cb, float ca, double fromX, double fromY, double toX, double toY, double rad, double samples, boolean topLeft, boolean topRight, boolean bottomLeft, boolean bottomRight) {
        BufferBuilder bufferBuilder = Tessellator.getInstance().getBuffer();
        bufferBuilder.begin(VertexFormat.DrawMode.TRIANGLE_FAN, VertexFormats.POSITION_COLOR);
